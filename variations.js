...
import {initAlbumChapter,refreshAlbumFocus} from './album.js';
...
function chapter(n,push=true){
  if(![1,2,3,4,5].includes(n))n=1;
  const two=n===2,three=n===3,four=n===4,five=n===5,first=el('.workspace'),second=el('#chapter-two'),third=el('#chapter-three'),fourth=el('#chapter-four'),fifth=el('#chapter-five');
  document.body.classList.toggle('chapter-two',two);document.body.classList.toggle('chapter-three',three);document.body.classList.toggle('chapter-four',four);document.body.classList.toggle('chapter-five',five);
  first.hidden=n!==1;second.hidden=!two;third.hidden=!three;fourth.hidden=!four;fifth.hidden=!five;first.inert=n!==1;second.inert=!two;third.inert=!three;fourth.inert=!four;fifth.inert=!five;
  const settings={
    ...
    4:{label:'<span>04</span> Cinemática glaciar',nav:'Las personas →',go:5,title:'Mocho · Cinemática glaciar'},
    5:{label:'<span>05</span> Las personas tras el dato',nav:'← Cinemática',go:4,title:'Mocho · Las personas tras el dato'}
  }[n];
  ...
  if(five)initAlbumChapter();
  const target=five?fifth:four?fourth:three?third:two?second:first,title=target.querySelector('h1');title.tabIndex=-1;title.focus({preventScroll:true});
  if(push&&!reduced)target.animate([{transform:`translateX(${n===1?-24:24}px)`,opacity:.5},{transform:'translateX(0)',opacity:1}],{duration:220});
  window.scrollTo(0,0);requestAnimationFrame(()=>{if(five)refreshAlbumFocus();else (four?getVelocityMap():three?getSnowMap():two?variationMap:window.mochoMap)?.resize();});
}
function syncChapter(){const match=location.hash.match(/^#capitulo-([12345])$/);chapter(match?+match[1]:1,false);}
